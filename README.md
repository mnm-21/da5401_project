...
│
├── plots/                         # Visualization outputs (committed to repo)
│   ├── data_exploration_summary.png
│   ├── correlation_heatmap.png
│   ├── per_metric_analysis.png
│   ├── text_length_script_analysis.png
│   └── [additional individual plots...]
│
├── artifacts/                     # Generated artifacts (gitignored - too large)
│   ├── train_clean.parquet        # Normalized training data
│   ├── test_clean.parquet         # Normalized test data
│   ├── per_metric_stats.csv       # Per-metric statistics
│   ├── feat_train_basic.parquet   # Feature matrices
...
├── submission_2_203.csv           # Earlier experimental submission (score: 2.203)
│
├── final_report/                  # Project report and documentation
│   ├── Project_report.tex         # LaTeX source for final report
│   ├── Project_report.pdf         # Compiled PDF report
│   ├── Project_report_plain_text.txt  # Plain text version 
│   └── plots/                     # Report-specific figures
│
├── README.md                      # This file
...
- **`phase0.ipynb`**: Initial data exploration, baseline models (LightGBM), and feature engineering. Includes:
  ...
  - Comprehensive data visualizations (saved to `plots/` directory)
...
- **`submission_2_203.csv`**: Earlier experimental submission (leaderboard score: 2.203). Included for comparison and reference.
...
The `plots/` directory contains high-resolution visualizations generated by `phase0.ipynb`, including:
- Score distribution analyses
- Per-metric statistics and distributions
- Text length and multilingual script analyses
- Feature correlation heatmaps
- Comprehensive data exploration dashboards

All plots are committed to the repository for easy viewing and documentation.

### Generated Artifacts

The `artifacts/` directory contains intermediate files generated during notebook execution:
- Normalized data splits (Parquet format)
- Feature matrices
- Per-metric statistics
- Model checkpoints and predictions

**Note**: This directory is gitignored due to large file sizes. The artifacts are regenerated automatically when running the notebooks.

### Final Report

The `final_report/` directory contains the complete project report and documentation:
- **`Project_report.tex`**: LaTeX source file for the final project report
- **`Project_report.pdf`**: Compiled PDF version of the report
- **`Project_report_plain_text.txt`**: Plain text extraction of the report
- **`plots/`**: Report-specific figures and visualizations

The report includes:
- Detailed methodology and design rationale
...
**Note**: The `artifacts/` directory will be created automatically during execution. The `plots/` directory already exists in the repository with pre-generated visualizations, but will be regenerated if you run `phase0.ipynb`.
...
3. The notebook will:
   ...
   - Create `artifacts/` directory for intermediate files (if it doesn't exist)
...
3. **File Not Found Errors**: Verify all files in `data/` directory exist and paths are correct. The `artifacts/` directory will be created automatically when running the notebooks.

4. **Missing Plots Directory**: If `plots/` directory doesn't exist, it will be created automatically when running `phase0.ipynb`. The directory is committed to the repository, so it should already exist.

5. **Different Results**: Ensure:
